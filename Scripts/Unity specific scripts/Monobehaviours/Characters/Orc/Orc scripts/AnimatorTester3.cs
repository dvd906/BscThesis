...
public class AnimatorTester3 : MonoBehaviour
{
    //For the editor
    ...
    void Start()
    {
        ...
        damageStep = DamagedAnimationsNumber > 2 ? 1 / DamagedAnimationsNumber : 1;
        attackStep = MeleeAttackAnimationsNumber > 2 ? (lastMeleeTreshold / MeleeAttackAnimationsNumber) : lastMeleeTreshold;
    ...
    private void UpdateAnimation(Animator characterAnimator)
    {
        if (!isDead)
        {
            ...
            if (Input.GetMouseButton(1) && canMove && !isAttacking)
            {
                ...
                {
                    characterAnimator.SetFloat(attackingAnimationHash, shootAnimationTreshold);
                    attackTime = shootTime;
                }
                else
                {
                    characterAnimator.SetFloat(attackingAnimationHash, attackingAnim);
                    attackingAnim = (attackingAnim + attackStep) > lastMeleeTreshold ? 0 : (attackingAnim + attackStep);
                    if (meleeAnimationTimes != null)
                    {
                        meleeIndex = ++meleeIndex < meleeAnimationTimes.Length ? meleeIndex : 0;
                        attackTime = meleeAnimationTimes[meleeIndex];
                    }
                    else
                    {
                        attackTime = 1.625f;
